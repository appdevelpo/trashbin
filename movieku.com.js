...
var CryptoJSAesJson = {
  ...
  async get_content(url) {
    ...
    // const script_match = res.match(/<script>.+?<\/script>/g);
    ...
    const subtitle_link  = decrypt_data.match(/https.+?srt/)
    ...
    // console.log(m3u8_res_list);
    // console.log(m3u8_source_list);
  ...
  async search(kw, page) {
    ...
    bsxList.forEach((element) => {
      const url = element.match(/href="https:\/\/107.152.37.223(.+?)"/)[1];
  ...
  async detail(url) {
    ...
    // console.log(res);
    console.log(title);
    const cover = res.match(/https:\/\/i\d.wp.com[^\?]+?"/)[0];
    console.log(cover);
    ...
    const ep_list = ep_list_area.match[0](/<li[\s\S]+?<\/li>/g);
    
    const urls = [];
    console.log(ep_list);
    ep_list.forEach((element) => {
      const url = element.match(/href="(.+?)"/)[1];
      const name = element.match(/<b>(.+?)<\/b>/)[1];
      urls.push({
        name,
        url
      })
